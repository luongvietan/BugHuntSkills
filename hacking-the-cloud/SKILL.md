...
- Found or stolen credentials: identify, validate, enumerate permissions, console access → `chapters/ch03-found-iam-credentials.md`
...
4. **Found credentials are radioactive.** `AKIA` keys live until revoked; `ASIA` keys expire in minutes-to-hours. Verify minimally (`get-caller-identity` once), screenshot the ARN, report immediately. Every API call beyond that is scope risk and CloudTrail noise.
...
All testing is assumed authorized and in scope (bug bounty / pentest). Restraint rules for cloud work: use your own AWS account for enumeration primitives; validate found keys once and stop; upload only harmless test files you own and delete them; never modify policies, create users/roles/keys, or run persistence techniques against a bounty target — document the *capability* (policy evidence) instead; treat post-exploitation chapters as "what an attacker could do" narrative for reports, not a to-do list. AWS's own policies additionally constrain testing against AWS infrastructure — check program rules for cloud-in-scope language.
