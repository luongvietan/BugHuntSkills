...
- **Outlive key deletion**: `sts:GetFederationToken --name x --duration-seconds 129600 --policy-arns arn:aws:iam::aws:policy/AdministratorAccess` mints ASIA creds that remain valid after the parent key is deleted (permissions = intersection of yours + the policy). Limitation: no IAM/STS calls via API — *except through a console session*.
