...
description: Deep vulnerability-mechanics knowledge base from "The Web Application Hacker's Handbook" (2nd ed., Stuttard & Pinto, Wiley 2011) — the "why does this attack work" reference. Use when hunting or explaining the core web vuln classes end-to-end: mapping attack surface, bypassing client-side controls, authentication, session management, access controls, SQL/XPath/LDAP/NoSQL injection, OS command injection, path traversal, file inclusion, XML/SOAP and back-end protocol injection, HTTP parameter injection/pollution, business-logic flaws, reflected/stored/DOM XSS with full filter-bypass technique, CSRF/clickjacking/cross-domain capture, open redirects, session fixation, custom attack automation and fuzzing, information disclosure, memory-safety bugs in native components, tiered/shared-hosting attacks, app-server and default-content flaws, source code review, tooling, and the full assessment methodology. Each chapter follows the book's signature: mechanism → attack → defense/evasion → checklists. All testing assumed authorized and in scope.
---
