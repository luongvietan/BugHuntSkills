...
- **0.CL request smuggling** — back-end ignores the request body length (treats CL as 0); the "body" is parsed as the next request. Front-end thinks it sent a clean request.
...
- **Connection warming / HTTP/1.1 keep-alive reuse** matters: browsers reuse connections, so the smuggled remainder reliably lands on the *same user's or next user's* request.
...
Escalation order for reports: parser anomaly → smuggle a complete request → poison the response queue / capture victim requests → steal session or deliver stored XSS/redirect to victims → ATO on shared infrastructure. HTTP/2 vectors often bypass front-end filtering that inspects valid HTTP/1 only.
